...
Every release also bumps the cache-bust query string (`?v=N`) on every CSS and JS import — including JS-internal imports inside modules. Cache-bust counter is independent of the app version (currently at `?v=22`).
...
```js
import { startLoading, showError } from '../ui-state.js?v=22';
